...
### Simultaneous representation of responents and words
...
To better render the graph, we limit the number of displayed labels by visualizing only the first 30 words contributing more to the first factorial plane, and we put the labels only on the words, while we plot only the "point" shape for the passengers.
...
In order to interpret the similarities among words, let'first plot them on the first factorial plane
...
-   the *1st axes* separates (on the right) words regarding the staff of the railway company form the rest of the words. Indeed on the right of the plot we have a cluster of words such as "staff", "english", "speaking", "crew", "speak", which are all related to the staff and, in particular, to their language skills.
...
That's why it may be useful to study word similarities also by looking at the biplot with the 3rd and 4th axes, which still capture more than 6% of the overall variance:
...
In order to find some similarities among the participants, we should add to the plot also the words: this will allow to group participants depending on the words they use inside their answers; this means that we'll find similar participants, according to the words they used in their answers to the openended question.

In order to do that, let's visualize side by side the 2 biplots (we could plot the biplot with the simultaneous representation of rows and columns, but it's not so clear due to the high number of overlapping), by considering only the first most contributing points :
...
-   Participants 2788 and 2963 are closely related to each other in the position of the word "less", this means that their answers include the word
...
-   We have a group of participants with a negative coordinate in the 3rd dimension that suggested improvements for what regards the "air conditioning" aspect: these users are, for instance, 2838, 3238, 3296, 3259, 3233)
...
Typically, hen doing CA for textual data, we shoul change the minimum threshold for the word frequency in order to verify the stability of the obtained representation, i.e. to see if we obtain a similar biplot.
...
In order to compare, we put the plots one after the other.

On the left we plot the biplot of the previous analysis, while on the right the new one:
...
-   even if the absolute words positioning changes (in the 2nd case the 1st factorial plane captures more than 13% of the overall variance), similar words are still cllosed to each other (e.g. "english and "staff", "air" and "conditioning", "size" and "space"
